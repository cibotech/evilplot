...
import com.cibo.evilplot.plotdefs.{LinePlotDef, OneLinePlotData}
...
class LinePlotSpec extends FunSpec with Matchers {
  ...
  describe("object LineToPlot") {
    ...
    val line2 = OneLinePlotData(points2, colors.red)

    val lines = Seq(line1, line2)

    it("has the right xBounds") {
      LinePlotDef(lines).xBounds shouldEqual Some(Bounds(-1, 7))
    }

    it("has the right yBounds") {
      LinePlotDef(lines).yBounds shouldEqual Some(Bounds(-2, 6))
    }
  }
  ...
  describe("LinesLater") {
    val lines = Seq(
      OneLinePlotData(Seq(Point(1, 2)), colors.forestGreen),
      OneLinePlotData(Seq(Point(3, 4)), colors.aquamarine))
  }
