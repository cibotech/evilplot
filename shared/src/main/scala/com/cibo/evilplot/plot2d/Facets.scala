...
object Facets {
  ...
  type FacetData = Seq[Seq[Plot2D[_]]]
  ...
  private def facetedPlotRenderer(plot: Plot2D[FacetData], extent: Extent): Drawable = {
  ...
  def apply(plots: Seq[Seq[Plot2D[_]]]): Plot2D[FacetData] = {
