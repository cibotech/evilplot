...
object SurfacePlot {
  ...
  private[plot] case class SurfacePlotRenderer(
    ...
    def render(plot: Plot[Seq[Seq[Point3]]], plotExtent: Extent): Drawable = {
      ...
      val xformedSurfaceBoundaries = plot.data.map(
        _.withFilter(p => plot.xbounds.isInBounds(p.x) && plot.ybounds.isInBounds(p.y))
...
object ContourPlot {
  ...
  def apply(data: Seq[Point],
    ...
    val gridData = KernelDensityEstimation
      .densityEstimate2D(data, gridDimensions, Some(xbounds), Some(ybounds))
    val binWidth = gridData.zBounds.range / contours
    val levels = Seq.tabulate(contours - 1)(bin => gridData.zBounds.min + (bin + 1) * binWidth)
    val contourPoints = levels.map(l => toPoint3(MarchingSquares.getContoursAt(l, gridData), l))
    Plot[Seq[Seq[Point3]]](contourPoints, xbounds, ybounds, SurfacePlotRenderer(SurfaceRenderer.contours()))
  }

  // MS implementation returns Seq[Segment], bridge to Seq[Point3] to avoid
  // breaking old plots (for now...)
  private def toPoint3(segments: Seq[Segment], level: Double): Vector[Point3] = {
    segments.flatMap(s => Vector(Point3(s.a.x, s.a.y, level), Point3(s.b.x, s.b.y, level))).toVector
  }
}
