...
object Label {
  ...
  trait LabelImplicits[T] {
    ...
    def title(label: String, size: Double = defaultTitleSize): Plot[T] = title(Text(label, size).padBottom(4))
    ...
    def leftLabel(label: String, size: Double = defaultLabelSize): Plot[T] =
      leftLabel(Text(label, size).rotated(270).padRight(4))
    ...
    def rightLabel(label: String, size: Double = defaultLabelSize): Plot[T] =
      rightLabel(Text(label, size).rotated(90).padLeft(4))

    def bottomLabel(d: Drawable): Plot[T] = plot :+ PlotLabel(PlotComponent.Bottom, d)
    def bottomLabel(label: String, size: Double = defaultLabelSize): Plot[T] = bottomLabel(Text(label, size).padTop(4))
