 ...
 *
  ...
  * @param cellExtent extent of cell
...
class GridLayout(numRows: Int, numCols: Int, toBeDrawn: Seq[DrawableLater], cellExtent: Extent,
  ...
  private val _drawable: Drawable = {
    (for (col <- 0 until numCols) yield {
      (for {
        row <- 0 until numRows
        _bottomPadding = if (row == 0) 0 else bottomPadding
