...
class BarChart(override val extent: Extent, data: Seq[Double], title: Option[String] = None, vScale: Double = 1.0)
  extends Drawable {
  ...
  private val barChart = Fit(extent) {
    ...
    val _yAxis = new YAxis(bars.extent.height, minValue, maxValue, vScale)
    val chart = _yAxis beside bars
    //val chart = bars
    title match {
  ...
  private class YAxis(height: Double, minValue: Double, maxValue: Double, vScale: Double) extends WrapDrawable {
    ...
    private val ticks = {
      val tickThick = 1
      val tickLength = 5
      val tickMaxScaled = tickMax * vScale
      val spacingScaled = spacing * vScale
      (for {
      // The graphics origin is in the top left, so we are walking the ticks in reverse order
        ...
        padTop = if (numTick == numTicks - 1)
          height - tickMaxScaled - (tickHeight / 2.0)
        else
